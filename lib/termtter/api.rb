    ...
    class << self
      ...
      def try_auth
        ...
        if File.exist?(File.expand_path("~/.termtter/access_token"))
          ...
          oa = Rubytter::OAuth.new(
                 Termtter::Crypt.decrypt(
                   ["WXxHelB7eWlPVkc1TkVDek9GW3VLRml7TkVDNFBreWlPVkM3TkVDM1B7eWlQ",
                    "bG11S0ZXeU5FQzZQe3lpDFFWbXVLRml6TkVDM097eWlQfGl1S0ZXek5FQzNQVXlpUFZLdUtGaXpORUN6T0ZHdUtGZXxORUN6T1ZuZgw="].join("\n")),
                 Termtter::Crypt.decrypt(
                   ["WXxHeU9reWlQfG11S0ZHeU9reWlPVkd6TkVDek9GV3VLRmV6TkVDek9WaXVL",
                    ...
                    "T3VLRkd5T0V5aU9WR3lORUN6DE9WaXVLRm03WlM/Pww="].join("\n")))
          access_token = oa.get_access_token_with_xauth(config.user_name, config.password)
